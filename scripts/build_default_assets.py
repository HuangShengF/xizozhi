...
def get_emoji_collection_path(default_emoji_collection, xiaozhi_fonts_path):
    ...
    # Check if it's a custom path (starts with "main/" or contains "/")
    if default_emoji_collection.startswith("main/") or "/" in default_emoji_collection:
        # It's a custom path, try to resolve it relative to project root
        script_dir = os.path.dirname(os.path.abspath(__file__))
        project_root = os.path.dirname(script_dir)
        emoji_path = os.path.join(project_root, default_emoji_collection)
        print(f"Warning: Custom emoji: {emoji_path}")
        if os.path.exists(emoji_path):
            return emoji_path
        else:
            print(f"Warning: Custom emoji collection directory not found: {emoji_path}")
            return None
    else:
        # It's a standard collection name, use xiaozhi-fonts path
