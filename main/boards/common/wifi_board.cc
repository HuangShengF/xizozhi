...
void WifiBoard::EnterWifiConfigMode() {
    ...
    // 播报配置 WiFi 的提示 (显示network二维码GIF)
    application.Alert(Lang::Strings::WIFI_CONFIG_MODE, "请扫码配网", "network", Lang::Sounds::OGG_WIFICONFIG);

    auto& blufi = Blufi::GetInstance();
    blufi.SsidSave([this](const std::string& ssid, const std::string& password) {
        auto& ssid_manager = SsidManager::GetInstance();
        ssid_manager.AddSsid(ssid, password);
        auto& board = Board::GetInstance();
        std::string data = board.GetBoardJson();
        Blufi::GetInstance().SendData(data.c_str(), data.length());
        ESP_LOGI(TAG, "save ssid");
    });

    blufi.WifiSettingOk([this]() {
        ESP_LOGI(TAG, "配网成功，准备重启...");
        xTaskCreate([](void* param) {
            ...
            app.Reboot();
            vTaskDelete(NULL);
        }, "reboot_task", 2048, NULL, 3, NULL);  // 优先级从5降低到3
    });
...
void WifiBoard::StartNetwork() {
    ...
    wifi_station.OnScanBegin([this]() {
        ...
        display->ShowNotification(Lang::Strings::SCANNING_WIFI, 20000);
    });
    wifi_station.OnConnect([this](const std::string& ssid) {
        ...
        notification += ssid;
        notification += "...";
        display->ShowNotification(notification.c_str(), 20000);
    });
    ...
    // 优化：缩短WiFi连接超时时间从60秒到15秒，快速反馈
    if (!wifi_station.WaitForConnected(45 * 1000)) {
        wifi_station.Stop();
        wifi_config_mode_ = true;
        EnterWifiConfigMode();
        return;
...
void WifiBoard::ResetWifiConfiguration() {
    ...
    // 使用统一的重启方法（停止音频服务 + 清理显示资源）
    ESP_LOGI(TAG, "使用统一重启流程");
    auto& app = Application::GetInstance();
    app.Reboot();
}
