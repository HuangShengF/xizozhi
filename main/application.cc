...
// #include "boards/mydazy-p30/config.h"
...
// hsf
// static void ScheduleStatusBarUpdate(Display* display, bool update_all) {
//     if (display == nullptr) {
//         return;
//     }
// #ifndef CONFIG_USE_EMOTE_MESSAGE_STYLE
//     lv_async_call([](void* arg) {
//         auto* ctx = static_cast<std::pair<Display*, bool>*>(arg);
//         ctx->first->UpdateStatusBar(ctx->second);
//         delete ctx;
//     }, new std::pair<Display*, bool>(display, update_all));
// #else
...
void Application::Reboot() {
    ...
    // Quiesce UI-related timers before tearing down the display tree.
    if (clock_timer_handle_ != nullptr) {
        esp_timer_stop(clock_timer_handle_);
    }
    if (status_timer_handle_ != nullptr) {
    ...
    // 清理显示资源（防止重启时黑屏）
    auto& board = Board::GetInstance();
    ESP_LOGI(TAG, "[reboot] before CleanupDisplay");
    board.CleanupDisplay();
    ESP_LOGI(TAG, "[reboot] after CleanupDisplay");

    vTaskDelay(pdMS_TO_TICKS(200));
    ESP_LOGI(TAG, "[reboot] before esp_restart");
    esp_restart();
}
